...
const ViewBuildingModal = ({ building, onClose, onUpdated }) => {
    ...
    const handleSave = async () => {
        ...
        try {
            await updateBuilding(building.id ?? building._id, { ...form, total_floors: Number(form.total_floors) || undefined });
            onUpdated();
        } catch (err) {
            ...
            <div className="glass-panel p-6" style={{ width: '100%', maxWidth: '420px', margin: '0 16px' }}>
                ...
                {!editMode ? (
                    <>
                        <div className="flex flex-col gap-3">
                            {[
                                ...
                                { label: 'Location', value: building.location_name || building.location_id || '—' },
                                { label: 'Total Floors', value: building.total_floors ?? '—' },
                                { label: 'Description', value: building.description || '—' },
                            ].map(({ label, value }) => (
                    ...
                    <div className="flex flex-col gap-4">
                        ...
                        <Input label="Total Floors" name="total_floors" type="number" value={form.total_floors} onChange={handleChange} />
